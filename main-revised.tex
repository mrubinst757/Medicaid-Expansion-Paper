...
%% Packages
\RequirePackage{amsthm,amsmath,amsfonts,amssymb,centernot,float,import,makeidx,subfiles, mathtools, epstopdf, hyperref, lscape, subcaption, accents, tabularx, threeparttable, varioref, natbib, graphicx}
...
%We conclude by noting the limitations of our variance estimation procedure. We motivate using regression-calibration by assuming the treated data is random and using the leave-one-state-out jackknife to estimate the variability as if we were re-sampling states from some larger super-population. Even in this setting our simulations suggest that these estimates can be conservative. Additionally, it may make more conceptual sense to view the treated data as fixed rather than random, and to evaluate the randomness in the estimates with respect to drawing new errors from the outcome model and new measurements. In this case we can still use regression-calibration approach to mitigate the bias from measurement error (see, e.g., \cite{gleser1992importance}). However, our variance estimates may again be overly conservative (see, e.g., \cite{buonaccorsi2010measurement}). Future work could consider how to better estimate the variance in this setting.
