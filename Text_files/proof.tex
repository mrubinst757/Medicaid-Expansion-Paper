...
Proposition~\ref{cl2} assumes that we know $\tilde{X}$, $\upsilon_1$, and $\bar{X}_0$; however, in practice we estimate it from the data. Moreover, the estimation of $\tilde{X}$ typically involves both the observed dataset and auxillary data, which we have not yet specified here. We give an example of this below.
...
%We conclude by analyzing the performance of GLS weights in the context of homoskedastic measurement error when the covariates are multivariate normal, but our model of $\tilde{X}$ treats the observations as independent -- i.e. $\tilde{X} = \upsilon_1 + \kappa^T(W_{sc} - \upsilon_1)$. 

%The set-up for this analysis requires some effort, but the results are simple: unless the covariates are uncorrelated, the GLS and H-SBW weights that reweight $\tilde{X}_{A=1}$ to $\bar{X}_0$ will be biased. However, the OLS weights will not be biased. Intuitively, this bias comes arises because the GLS and H-SBW weights depend on an incorrect estimate of $X_{sc}X_{sd}^T$, while the OLS and SBW weights do not have this dependence. We demonstrate this by specifying an example of correlated data and analyzing the resulting asymptotic imbalance in Proposition~\ref{cl10}.

%Assume $X_{sc} \sim MVN(\mu_j, \Sigma_X)$ and $\mu_j \sim MVN(\upsilon_1, \Sigma_S)$. Therefore $Cov(X_{sc}, X_{s'c'}) = \Sigma_S\mathds{1}(s = s', c \ne c') + \Sigma_X\mathds{1}(s = s', c = c')$. Together with our measurement error models, we conclude that $Cov(W_{sc}, W_{s'c'}) = \Sigma_S\mathds{1}(s = s', c \ne c') + \Sigma_W\mathds{1}(s = s', c = c')$ and that $Cov(W_{sc}, X_{s'c'}) = \Sigma_S\mathds{1}(s = s', c \ne c') + \Sigma_X\mathds{1}(s = s', c \ne c')$. We assume throughout that each state has $p_s$ regions, and for simplicity we further assume that $p_s$ is constant across all states -- i.e. $p_s = p$.

%We then have the entire data $X = (X_{11}, ..., X_{1p_1}, ..., X_{sp_s})$ reflects a single draw from the distribution $MVN(\mathbf{\upsilon}_1, \mathbf{\Sigma}_X)$, where $\mathbf{\upsilon}_1$ is a $qn_1$ dimensional vector that repeats the $q$ dimensional vector of means $\upsilon_1$ defined previously $n_1$ times. Meanwhile $W = W_{11}, ..., W_{1p_1}, ..., W_{sp_s})$ reflects a single draw from the distribution $N(X, \Sigma_{\nu})$. Let $\Sigma_W = \Sigma_W + \Sigma_{\nu}$. $(X, W)$ are then jointly normal with covariance matrix $\Sigma$: 
...
%\end{align*}
%
%Notice that $\mathbf{\Sigma}_X$ and $\mathbf{\Sigma}_W$ are block-diagonal matrices with $qp_s$ by $qp_s$ blocks indexed by $b_s$ $\Sigma_{X, b_s}$ and $\Sigma_{W, b_s}$, with each element containing a $q$ by $q$ matrix: 
...
%\end{align*}
%
%Let $\mathbf{\kappa} = \mathbf{\Sigma_W}^{-1}\mathbf{\Sigma_X}$. We then have that by the conditional normal distribution:

%\begin{align*}
%    \mathbb{E}[X \mid W, A = 1] &= \tilde{X}^\star =  \mathbf{\upsilon}_1 + \mathbf{\kappa}^T(W - \mathbf{\upsilon}_1)
%\end{align*}
%
%As a special case, notice that if $\Sigma_S = 0$ (i.e. the data is uncorrelated), then $\mathbf{\kappa}$ is diagonal with elements $\Sigma_W^{-1}\Sigma_X$, which returns us to the adjustment proposed in \eqref{eqn:regcal}. This is therefore a simple generalization of \eqref{eqn:regcal} that accounts for positive equicorrelation among the covariates.

%We consider the case where instead of reweighting $\tilde{X}^\star$, we reweight $\tilde{X}$ (as defined in \eqref{eqn:regcal}) to known target $\bar{X}_0$. Without loss of generality, we assume throughout that $\upsilon_1 = 0$.

%\begin{proposition}\label{cl10}
%    Let $\Omega$ be block-diagonal with $p_s$ by $p_s$ blocks for each state containing $1$ in the diagonal elements and $\rho \in [0, 1)$ in the off-diagonals. The GLS weights that fall in the constraint set $\Gamma_U(\tilde{X}_{A=1}, \bar{X}_0, 0)$ do not balance the true covariates asymptotically in expectation unless one of the following conditions holds:
%    \begin{itemize}
%    \item $\Sigma_S = 0$; that is, $X$ is uncorrelated across regions
%    \item $p_s = 1$; that is, there is only one region per state (note that this forces the first condition to be true)
%    \item $\kappa = I_q$; that is, there is no measurement error
%    \item $\Omega = I_n$; that is, the GLS weights are OLS weights
%    \end{itemize}
%\end{proposition}

%\begin{proof}
%    Let $\tilde{X} = W_{A=1}\kappa$. Consider $\hat{\gamma}_{sc} = \bar{X}_0^T(\tilde{X}^T\Omega^{-1}\tilde{X})^{-1}(\tilde{X}\Omega^{-1})_{sc}$. We want to derive an expression for 
    
%    \begin{align*}
%        \mathbb{E}[\sum_{s=1}^m\sum_{c=1}^p\hat{\gamma}_{sc}X_{sc}] &= \mathbb{E}[\sum_{s=1}^m\sum_{c=1}^pX_{sc}(\bar{X}_0^T(\tilde{X}^T\Omega^{-1}\tilde{X})^{-1}(\tilde{X}\Omega^{-1})_{sc})] \\ 
%        &= \mathbb{E}[\underbrace{\bar{X}_0^T(\tilde{X}^T\Omega^{-1}\tilde{X})^{-1}}_{T_1}\underbrace{\sum_{s=1}^m\sum_{c=1%}^pX_{sc}(\tilde{X}\Omega^{-1})_{sc}^T}_{T_2}]
%    \end{align*}
    
%    We then obtain:

%    \begin{align*}
%        \mathbb{E}[n^{-1}T_1^{-1}] = \mathbb{E}[n^{-1}\tilde{X}^T\Omega^{-1}\tilde{X}] &= n^{-1}\sum_{s=1}^m\sum_{c=1}^p\mathbb{E}[a_1\tilde{X}_{sc}\tilde{X}_{sc}^T + \sum_{c\ne d}a_2\tilde{X}_{sc}\tilde{X}_{sd}^T] \\ &=n^{-1}\sum_{s=1}^m\sum_{c=1}^p\mathbb{E}[\kappa^Ta_1W_iW_i^T\kappa + \sum_{c\ne d}a_2\kappa^TW_iW_i^T\kappa] \\
%        &= n^{-1}\sum_{s=1}^m \kappa^T(a_1p\Sigma_{WW} + p(p - 1)a_2\Sigma_{SS})\kappa]\\
%        &= a_1 \Sigma_{XX} \kappa + a_2(p - 1)\kappa^T \Sigma_{SS} \kappa  \\
%        \mathbb{E}[n^{-1}T_2] = \mathbb{E}[n^{-1}\sum_{s=1}^m\sum_{c=1}^pX_{sc}(\tilde{X}\Omega^{-1})_{sc}^T] &= n^{-1}\sum_{s=1}^m\sum_{c=1}^p\mathbb{E}[X_{sc}(a_1\tilde{X}_{sc}^T + a_2\sum_{c\ne d}\tilde{X}_{sd}^T)] \\
%        &= a_1\Sigma_{XX}\kappa + a_2(p-1)\Sigma_{SS}\kappa
%    \end{align*}
    
%    A sufficient condition for these weights to asymptotically balance $X_{sc}$ to $\bar{X}_0$ is if $\mathbb{E}[T_1T_2] \to^p I_q$. By Slutsky's Theorem, we see that this does not hold unless one of the aforementioned conditions are met.
%\end{proof}

%\begin{remark}
%    We see that balance is not achieved asymptotically in expectation unless $\kappa = I_q$, $\Sigma_S = 0$, $p_s = 1$, or $a_2 = 0$ (i.e. $\Omega = I_n$). In other words, we cannot achieve balance unless (1) there is no measurement error; (2) $X_{sc}$ and $X_{sd}$ are uncorrelated; (3) there is only one county per state; (4) $\Omega$ is a diagonal matrix (which implies that $a_2 = 0$). 
%\end{remark}

%\begin{remark}
%    While for simplicity we assumed constant units per state, we conjecture that we could extend this result for a random $p_s$. However, doing so is beyond the scope of this paper, as this example is only meant to shed light on the source of this bias. 
%\end{remark}

%\begin{remark}
%    We derived the exact expression of the expected asymptotic imbalance for OLS and GLS weights, but not SBW or H-SBW weights. As we noted above in Proposition~\ref{cl9}, the H-SBW weights are equivalent to the GLS weights on some subset of the data. While the exact bias may differ (for example, $a_1$ and $a_2$ would then vary by state, since in general $p$ would no longer be constant for each state), the source of this bias -- the dependence of the weights on the correlation structure of the data -- also applies to the H-SBW weights optimized over the constraint set $\Gamma(\tilde{X}, \bar{X}_0, 0)$.
%\end{remark}
