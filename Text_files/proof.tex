...
%\begin{proof}[Max's proof idea for dependent SBW]
%Claim: even if data are dependent, for SBW it suffices to balance on $\tilde{X}_{sc} = \mathbb{E}[X_{sc} \mid W_{sc}]$ rather than $\mathbb{E}[X_{sc} \mid W]$. (need to rewrite as a formal statement). 

%Proof: Will show that $\mathbb{E}[\sum \gamma_{sc}X_{sc}] = \sum \gamma_{sc} \mathbb{E}[X_{sc} \mid W_{sc}]$ for SBW weights $\gamma$ optimized across the constraint set $\Gamma(\tilde{X}, \bar{W}_0, 0)$.

%Let $S$ denote a random variable denoting which state each CPUMA belongs to such that $S = s \Leftrightarrow X_i = X_{sc}$ where $c$ is an arbitrary index of CPUMAs within states. Therefore that $(X_i \mid S = s) = X_{sc}$. Moreover, assume that $X_{sc} \mid S = s \stackrel{iid}\sim N(\mu_s, \Sigma)$. Recall that $W_{sc} \mid X_{sc} \stackrel{iid}\sim N(X_{sc}, \Sigma_\nu)$. Then we have that:

%\begin{align*}
%    \mathbb{E}[\sum_{i=1}^{n_1} \gamma_iX_i] &= \mathbb{E}[\sum_{i=1}^{n_1} \mathbb{E}[\gamma_iX_i \mid S]] \\
%    &= \mathbb{E}[\sum_{i=1}^{n_1} \mathbb{E}[\mathbb{E}[\gamma_iX_i \mid W, S] \mid S]] \\
%    &= \mathbb{E}[\sum_{i=1}^{n_1} \mathbb{E}[\mathbb{E}[\gamma_iX_i \mid W_i, S] \mid S]] \\ %bc W are iid given S 
%    &= \mathbb{E}[\sum_{i=1}^{n_1} \mathbb{E}[\mathbb{E}[\gamma_iX_i \mid W_i] \mid S]] \\ %bc optimization doesn't depend on S
%    &= \mathbb{E}[\sum_{i=1}^{n_1} \mathbb{E}[\gamma_i\mathbb{E}[X_i \mid W_i] \mid S]] \\
%    &= \mathbb{E}[\sum_{i=1}^{n_1} \mathbb{E}[\gamma_i\mathbb{E}[X_i \mid W_i]] \\
%\end{align*}

%In the case of SBW we have that $\mathbb{E}[\gamma \mid S] = \mathbb{E}[\gamma]$ (by noting that $S$ is not an input to the SBW objective):

%\begin{align*}
%    \implies \mathbb{E}[\sum_{i=1}^{n_1} \mathbb{E}[\gamma_i\mathbb{E}[X_i \mid W_i] \mid S]] &= %\mathbb{E}[\sum_{i=1}^{n_1} \gamma_i\mathbb{E}[\mathbb{E}[X_i \mid W_i] \mid S] \\
%    &= \mathbb{E}[\sum_{i=1}^{n_1} \gamma_{i}\mathbb{E}[X_i \mid W_i]] \\
%\end{align*}

%More generally let $S$ denote a variable satisfying $X_i \mid S_i \stackrel{iid}\sim N(S_i, \Sigma)$. The logic above still holds.

%This concludes the proof. Notice that for H-SBW:

%\begin{align*}
%    \mathbb{E}[\sum_{i=1}^{n_1} \gamma_iX_i] &= \mathbb{E}[\sum_{i=1}^{n_1} \mathbb{E}[\gamma_iX_i \mid S, W]] \\
