...
\begin{remark}
If we view $X$ as fixed and substitute $\bar{X}_a$ for $\upsilon_a$ (again assuming quantity is known), the remaining term contributes to a finite-sample bias rather than variance. Moreover, the squared-bias term will decrease with the variance of the weights.
\end{remark}
...
We show that the H-SBW estimator minimizes the variance of any estimator within the constraint set given a constant within-group correlation structure. Here we assume that the true covariates are measured.
...
Then consider the variance of any weighting estimator using weights $\gamma$ where $\gamma^T1 = n_t$.
...
\begin{align}
    ...
    &\propto n_t^{-2}\sum_{s: A_s = 1}(\sum_{c = 1}^{p_s}(\gamma_{sc}^2 + \sum_{c \ne d}\rho \gamma_{sc}\gamma_{sd})
\end{align}
